...
[![Runpod](https://api.runpod.io/badge/Sajib-Hossain-Cloud/OSS-VLLM)](https://console.runpod.io/hub/Sajib-Hossain-Cloud/OSS-VLLM)

Deploy OpenAI-Compatible Blazing-Fast LLM Endpoints with **GPT-OSS** and **MXFP4** support, powered by [vLLM](https://github.com/vllm-project/vllm) on RunPod Serverless.
...
- [Deploying GPT-OSS Models](#deploying-gpt-oss-models-gpt-oss-20b--gpt-oss-120b)
...
| `TOOL_CALL_PARSER`                  | Parser for tool calls                             |                     | "openai", "mistral", "hermes", "llama3_json", "granite", "deepseek_v3", etc. |
...
OpenAI's GPT-OSS models are fully supported with optimized MXFP4 quantization. These models feature:
...
Deploy using the pre-built Docker image with these environment variables:
...
<details>
<summary>H100/H200 (Hopper)</summary>
...
<details>
<summary>B200 (Blackwell)</summary>
...
### Responses API (Recommended for GPT-OSS)

The Responses API provides better support for GPT-OSS features like reasoning and tool use:
...
GPT-OSS models use MXFP4 (Microscaling FP4) quantization for the MoE layers, which is built into the model weights. You don't need to specify a separate quantization method - just deploy the model and the optimized kernels will be used automatically.

For other models that support MXFP4, you can specify:
```bash
QUANTIZATION=mxfp4
```

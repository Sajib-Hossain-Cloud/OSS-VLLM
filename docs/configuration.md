...
| `QUANTIZATION`                 | None                | ['awq', 'squeezellm', 'gptq', 'bitsandbytes', 'mxfp4']      | Method used to quantize the weights. Note: GPT-OSS models have MXFP4 built-in.  |
...
## GPT-OSS and MXFP4 Settings

Settings specific to running OpenAI's GPT-OSS models (gpt-oss-20b, gpt-oss-120b) which use MXFP4 quantization.

| Variable                              | Default | Type/Choices | Description                                                                                                  |
| ------------------------------------- | ------- | ------------ | ------------------------------------------------------------------------------------------------------------ |
| `ASYNC_SCHEDULING`                    | `true`  | `bool`       | Enable asynchronous scheduling for better GPU utilization. Recommended for GPT-OSS models.                   |
| `NO_ENABLE_PREFIX_CACHING`            | `true`  | `bool`       | Disable prefix caching. Recommended for GPT-OSS models for consistent performance.                           |
| `MAX_CUDAGRAPH_CAPTURE_SIZE`          | None    | `int`        | Maximum size for CUDA graphs. Recommended: 2048 for GPT-OSS models.                                          |
| `COMPILATION_CONFIG`                  | None    | `str` (JSON) | vLLM compilation config. For Blackwell: `{"pass_config":{"fuse_allreduce_rms":true,"eliminate_noops":true}}` |
| `TOOL_SERVER`                         | None    | `str`        | MCP tool server URL(s) for GPT-OSS built-in tools. Use `demo` for demo tools or comma-separated URLs.        |
| `VLLM_USE_FLASHINFER_MOE_MXFP4_MXFP8` | None    | `int`        | Set to `1` to enable FlashInfer MXFP4+MXFP8 MoE kernels on Blackwell GPUs. Auto-detected for GPT-OSS.        |

### Hardware-Specific Recommendations for GPT-OSS
...
NO_ENABLE_PREFIX_CACHING=true
MAX_CUDAGRAPH_CAPTURE_SIZE=2048
COMPILATION_CONFIG='{"pass_config":{"fuse_allreduce_rms":true,"eliminate_noops":true}}'
```
...
| `TOOL_CALL_PARSER`                  | `None`      | `str`            | Specifies the parser for tool calls. Options: `openai`, `mistral`, `hermes`, `llama3_json`, `llama4_json`, `llama4_pythonic`, `granite`, `granite-20b-fc`, `deepseek_v3`, `internlm`, `jamba`, `phi4_mini_json`, `pythonic`. Use `openai` for GPT-OSS models. |

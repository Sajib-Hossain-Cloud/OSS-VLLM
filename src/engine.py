...
class OpenAIvLLMEngine(vLLMEngine):
    ...
    async def _initialize_engines(self):
        ...
        if RESPONSES_API_AVAILABLE:
            try:
                ...
                logging.warning(f"Failed to initialize Responses API engine: {e}")
    ...
    async def _handle_responses_request(self, openai_request: JobInput):
        ...
        dummy_request = DummyRequest()
        response_generator = await self.responses_engine.create_response(request, raw_request=dummy_request)
